...
export default function WorkoutLogPage() {
  ...
  function handleCompleteSet(setIndex) {
    ...
    const exerciseSets = updatedSets.filter(s => s.exerciseIndex === exerciseIndex);
  ...
  function handleSaveAndExit() {
    console.log('Progress saved:', sets.filter(s => s.completed));
    router.push('/program');
  }
  ...
  const exerciseSets = sets.filter(s => s.exerciseIndex === currentExerciseIndex);
    ...
    <div className="workout-log-page">
      <div className="container">
        ...
        <div className="workout-header">
          <a href="/program" className="back-button">← Back to Program</a>
          <h1 className="workout-title">{workout.workoutName}</h1>
          <p className="workout-subtitle">Day {workout.dayName} • {workout.subtitle}</p>

          <div className="progress-tracker">
            <div className="progress-item">
              <div className="progress-label">Exercises</div>
              <div className="progress-value">{currentExerciseIndex + 1}/{workout.exercises.length}</div>
            </div>
            <div className="progress-item">
              <div className="progress-label">Sets</div>
              <div className="progress-value">{completedSets}/{totalSets}</div>
            </div>
            <div className="progress-item">
          ...
          <div className="xp-progress">
            <div className="xp-header">
              ...
              <span style={{color: '#10b981', fontWeight: 600}}>{xpEarned} / {workout.totalXp} XP</span>
            </div>
            <div className="xp-bar-bg">
              <div className="xp-bar-fill" style={{width: `${Math.min((xpEarned / workout.totalXp) * 100, 100)}%`}}></div>
            </div>
          </div>
        </div>
        ...
        {restTimer !== null && restTimer > 0 && (
          <div className="rest-timer">
            ...
            <div className="timer-buttons">
              <button className="timer-button" onClick={() => setRestTimer(restTimer - 30)}>-30s</button>
              <button className="timer-button" onClick={() => setRestTimer(180)}>Reset</button>
              <button className="timer-button" onClick={() => setRestTimer(restTimer + 30)}>+30s</button>
              <button className="timer-button" onClick={() => setRestTimer(0)}>Skip Rest</button>
            </div>
          </div>
        )}
          ...
          return (
            ...
            >
              <div className="exercise-header">
                ...
                <span className={`exercise-badge ${isActive ? 'badge-active' : isExComplete ? 'badge-completed' : 'badge-upcoming'}`}>
                  {isActive ? 'Current Exercise' : isExComplete ? '✓ Completed' : 'Upcoming'}
                </span>
              </div>
              ...
              {!isUpcoming && (
                <ul className="sets-list">
                  {exSets.map((set, idx) => {
                    const globalSetIndex = sets.findIndex(s =>
                      s.exerciseIndex === exIdx && s.setNumber === set.setNumber
                    );
                    const isCurrentSet = globalSetIndex === currentSetIndex;

                    return (
                      <li key={idx} className={`set-item ${set.completed ? 'completed' : ''}`}>
                        <div className="set-number">{set.setNumber}</div>
                        <div className="set-inputs">
                          <div className="input-group">
                            <div className="input-label">Weight ({exercise.unit || 'bodyweight'})</div>
                            <input
                              type="number"
                              className="set-input"
                              value={set.weight}
                              onChange={(e) => handleUpdateSet(globalSetIndex, 'weight', e.target.value)}
                          ...
                          <div className="input-group">
                            ...
                            <input
                              ...
                              className="set-input"
                              value={set.reps}
                              onChange={(e) => handleUpdateSet(globalSetIndex, 'reps', e.target.value)}
                              disabled={set.completed || !isCurrentSet}
                            />
                          </div>
                        </div>
